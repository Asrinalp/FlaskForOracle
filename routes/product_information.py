...
def add_product():
    ...
    if request.method == 'POST':
        ...
        query = '''
        BEGIN
            -- Foreign key check manually (optional)
            DECLARE
                v_category_exists INT;
                v_subcategory_exists INT;
                v_supplier_exists INT;
        BEGIN
        -- Check if foreign keys exist
            ...
            SELECT COUNT(*) INTO v_subcategory_exists
        ...
        -- Raise an exception if a foreign key does not exist
            IF v_category_exists = 0 THEN
                RAISE_APPLICATION_ERROR(-20001, 'Category ID does not exist.');
            ELSIF v_subcategory_exists = 0 THEN
                RAISE_APPLICATION_ERROR(-20002, 'Subcategory ID does not exist.');
            ELSIF v_supplier_exists = 0 THEN
                RAISE_APPLICATION_ERROR(-20003, 'Supplier ID does not exist.');
            ELSE
                -- Proceed with the insert if all keys are valid
    ...
    cursor.execute('SELECT DISTINCT category_id, category_name FROM prod_category')
    categories = cursor.fetchall()
    cursor.execute('SELECT DISTINCT subcategory_id, subcategory_name FROM prod_subcategory')
    subcategories = cursor.fetchall()
    cursor.execute('SELECT DISTINCT supplier_id, supplier_name FROM supplier')
...
def edit_product(product_id):
    ...
    if request.method == 'POST':
        ...
        # Update the product information -> Category / subcategory/ supplier -> var mı diye kontrol ediyor 
        update_query = '''
            BEGIN
            -- Declare variables for foreign key checks
            DECLARE
                v_category_exists INT;
                v_subcategory_exists INT;
                v_supplier_exists INT;
            ...
            BEGIN
                ...
                ELSIF v_supplier_exists = 0 THEN
                    RAISE_APPLICATION_ERROR(-20003, 'Supplier ID does not exist.');
                ELSE
                    -- Proceed with the update if all keys are valid
                    ...
                    DBMS_OUTPUT.PUT_LINE('Product information updated successfully for product ID: ' || :product_id);
                END IF;
            END;

...
def add_employee():
    ...
    # Departman bilgileri listeleme için
    cursor.execute('SELECT department_id, department_name FROM departments')
    departments = cursor.fetchall()

    # Yöneticiler bilgisi listeleme için
    cursor.execute('''
        SELECT e.employee_id, e.first_name || ' ' || e.last_name AS manager_name
        FROM employees e
        WHERE e.employee_id IN (SELECT DISTINCT manager_id FROM employees WHERE manager_id IS NOT NULL)
    ...
    # Job bilgilerini listeleme için
    cursor.execute('SELECT job_id, job_title FROM jobs')
    jobs = cursor.fetchall()
    ...
    if request.method == 'POST':
        ...
        employee_data = {
            ...
            'job_id': request.form['job_id'],  # 'job_id' alıyoruz ama artık job_name ile gösteriyoruz.
        ...
        try:
            hire_date = datetime.strptime(hire_date_str, '%Y-%m-%d')  # Örnek format: 'YYYY-MM-DD'
            employee_data['hire_date'] = hire_date
        except ValueError as e:
            current_app.logger.error(f"Invalid date format for hire_date: {e}")
            hire_date = None  # Hata durumunda None döndürülebilir ya da farklı bir işlem yapılabilir.

        cursor.execute('''
            INSERT INTO employees (
                employee_id, first_name, last_name, email, 
                phone_number, job_id, salary, manager_id, department_id, hire_date
            ) VALUES (
                :employee_id, :first_name, :last_name, :email,
                :phone_number, :job_id, :salary, :manager_id, :department_id, :hire_date
            )
        ...
        conn.commit()
        conn.close()
        current_app.logger.info(f"Employee {employee_data['first_name']} {employee_data['last_name']} added by user {session['email']}.")
